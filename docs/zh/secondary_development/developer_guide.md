...
|crossbuild-essential-arm64|（可选，仅限于X86编译环境需要）交叉编译依赖工具包。|执行以下命令，安装交叉编译依赖工具包。<li>openEuler<ul>```yum install gcc-aarch64-linux-gnu g++-aarch64-linux-gnu binutils-aarch64-linux-gnu```</ul></li><li>Ubuntu<ul>```apt-get install crossbuild-essential-arm64```</ul></li>|
...
3. 在customized\_alarm\_check.c中实现自定义告警初始化接口和回调函数注册接口。
    ...
    unsigned char *generate_customized_alarm()
    {
        ...
        (void)memset_s(alarmBuff, head_size + body_size, 0, head_size + body_size);
...
6. 校验配置文件是否配置正确。
    ...
    2. 执行以下命令，校验配置文件。
        ...
        def check_alarm_shield_config(content: dict, filename: str) -> Result:
            """
            检查告警屏蔽配置文件是否正确，检查内容包括是否存在重复的innerid、是否存在重复的告警id+主体、告警屏蔽配置格式是否正确以及字段是否缺失和字段不在要求的字段范围内
            :param content:配置文件内容
        ...
        def check_alarm_shield_web_config(content: dict, filename: str) -> Result:
            """
            检查前端告警屏蔽配置文件是否正确，检查内容包括是否存在重复的innerid、是否存在重复的告警id+主体、告警屏蔽配置格式是否正确以及字段是否缺失和字段不在要求的字段范围内
            :param content:配置文件内容
            :param filename:配置文件名
            :return:检测结果
            """
...
|setting.safety|含义：Web界面导航栏是否显示“安全策略”页签<br>类型：bool<br>取值：<li>true：显示该模块</li><li>false：不显示该模块</li><div class="note"><span class="notetitle">[!NOTE] 说明</span><div class="notebody">安全策略可以动态选择是否显示，若显示则安全策略的所有功能将一起显示，若不显示，则安全策略的所有功能将一起隐藏。</div></div>|
...
|type|必选|数据类型，支持以下类型：<li>string </li><li>int</li><li>long long</li><li>float</li><li>bool</li><li>json</li>|
|accessMode|必选|访问模式。<li>Read：只读</li><li>ReadWrite：读写</li><li>Write：只写</li><li>ReadHide：只读，在Web界面上不会明文展示该属性值</li><li>WriteHide：只写，在Web界面上不会明文修改该属性值</li><li>ReadWriteHide：读写，在Web界面不会明文展示和修改该属性值</li><div class="note"><span class="notetitle">[!NOTE] 说明</span><div class="notebody"><li>建议涉及到敏感数据的属性使用带有Hide的访问模式。</li><li>type为bool的属性，不能设置为带有Hide的访问模式。</li></div></div>|
|description|可选|属性说明。|
...
4. 在demo\_module.c中实现接口及其关键字段。
    ...
    /* json类型数据获取封装接口，将TLV编码解析的value进行TLV解析以完成json属性获取 */
    ...
    /* 可热插拔设备的模组接口，用于获取指定设备类型的设备列表。当模组信息实时变化的情况下，使用此接口刷新模组设备信息 */
    int dev_get_device_list(int type, unsigned int buffSize, unsigned char *buff, unsigned int device_name_len)
    {
        if (g_demo_ctrl == NULL) {
            return -1;
...
3. 使用配置文件检查脚本validate\_device\_config.py对修改后的配置文件进行检查，验证配置文件是否正确。

    ```bash
    cd src/app/add_extend_device
    python3 validate_device_config.py
    ```
...
<tbody><tr id="row19520181885415"><td class="cellrowborder" valign="top" width="49.96%" headers="mcps1.2.3.1.1 "><p id="p17520618115412"><a name="p17520618115412"></a><a name="p17520618115412"></a>om-sdk.tar.gz</p>
</td>
...
- version.xml文件示例如下：
    ...
    **表 4**  参数说明
    ...
    <td class="cellrowborder" rowspan="8" valign="top" width="50%" headers="mcps1.2.3.1.2 "><p id="p10328185711514"><a name="p10328185711514"></a><a name="p10328185711514"></a><span id="ph1335062845811"><a name="ph1335062845811"></a><a name="ph1335062845811"></a>边缘管理系统</span>软件正常运行的必要配置，不可配置和修改</p>
    </td>
    </tr>
...
2. 按照如下示例，编写构建脚本**build\.sh**，脚本路径为“_\{project\_dir_\}/build/build.sh”。
    ...
    function build_package()
    {
        ...
        # build_signature.sh需用户自行实现，该实现仅为示例
        build_sign_file
        # 重新打包omsdk.zip
        zip_omsdk_package
        return 0
