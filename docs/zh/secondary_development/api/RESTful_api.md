...
|AutoRefresh|可选|用于更新会话超时起始记录时间。|字符类型，取值为"true"或者"false"。为"true"时不会更新会话超时起始记录时间，为"false"时会更新会话超时起始记录时间为当前系统运行时间的时间戳。<div class="note"><span class="notetitle">[!NOTE] 说明</span><div class="notebody">不传此参数时默认为true。</div></div>|
...
### 查询当前根服务资源<a name="ZH-CN_TOPIC_0000001578489844"></a>
...
|@odata.id|字符串|当前任务资源的访问路径。<br><div class="note"><span class="notetitle">[!NOTE] 说明</span><div class="notebody">您可以访问该资源，获取该任务的详细信息。</div></div>|
...
接口已默认开放，但功能暂未完全实现，需要开发者适配恢复出厂设置命令。
...
|Name|字符串|系统时间的名称。|
|Id|字符串|系统时间的ID。|
|Datetime|字符串|系统当前时间。<br><div class="note"><span class="notetitle">[!NOTE] 说明</span><div class="notebody">此接口可获取精度较高的系统时间。</div></div>|
...
|state_data|必选|移动数据的开关状态。|布尔值<li>true：打开移动数据。</li><li>false：关闭移动数据。</li><div class="note"><span class="notetitle">[!NOTE] 说明</span><div class="notebody">当state_lte取值为false时，state_data取值为true无效。</div></div>|
...
|auth_type|必选|身份验证类型。|字符串。取值为0、1、2、3，其含义分别为：<li>0：NONE</li><li>1：PAP</li><li>2：CHAP</li><li>3：PAP or CHAP</li><div class="note"><span class="notetitle">[!NOTE] 说明</span><div class="notebody">4G模块支持的auth_type取值为0、1、2；5G模块支持的auth_type的取值为0、1、2、3。</div></div>|
...
|RemoteTestIp|可选|对新增或修改的IP地址进行连通性测试的远端IP地址|字符串，IPv4地址。注意不能和Address参数的配置相同。<br><div class="note"><span class="notetitle">[!NOTE] 说明</span><div class="notebody">如果要配置ConnectTest和RemoteTestIp，两个参数必须同时配置。只有当ConnectTest选择True时，此字段才有效，Web端新增IP时默认需要进行连通性测试。</div></div>|
...
|@odata.id|字符串|当前任务资源的访问路径。<div class="note"><span class="notetitle">[!NOTE] 说明</span><div class="notebody">您可以访问该资源，获取该任务的详细信息。</div></div>|
...
|NodeId|是否必选取决于ManagerType<li>ManagerType为Web时：可选</li><li>ManagerType为FusionDirector时：必选</li>|节点ID|字符串，当前仅支持一种格式：UUID，长度为36字节。如e6a47e30-3a09-11ea-9218-a8494df5f123。<br><div class="note"><span class="notetitle">[!NOTE] 说明</span><div class="notebody">字符取值为数字（0\~9）、中划线（-）、小写字符（a\~f）有效。</div></div>|
...
|CapacityBytes|必选|容量大小 <div class="note"><span class="notetitle">[!NOTE] 说明</span><div class="notebody">磁盘分区创建中，空间大小采用2M向上对齐，当分区的容量不是2M的倍数时向上对齐。</div></div>|字符串类型，取值为数字，单位GB，最小值0.5，且最多一位小数，必须是0.5的整数倍。|
...
|AccountInsecurePrompt|布尔值|账号不安全提示。<li>true：密码为默认密码。</li><li>false：密码不是默认密码。</li><div class="note"><span class="notetitle">[!NOTE] 说明</span><div class="notebody">默认密码时仅允许登录并修改密码，其余接口均不可访问。</div></div>|
...
|Uptime|字符串|系统连续运行时间。|
|Datetime|字符串|系统当前时间。<br><div class="note"><span class="notetitle">[!NOTE] 说明</span><div class="notebody">由于系统数据缓存，返回的时间与系统时间存在20秒左右的偏差。</div></div>|
...
|DateTime|可选|系统当前时间。|字符串。长度为10~64字符，时间格式：%Y-%m-%d %H:%M:%S或者是%Y-%m-%d。<br><div class="note"><span class="notetitle">[!NOTE] 说明</span><div class="notebody">系统支持设置的时间范围为Linux系统支持的时间范围。</div></div>|
...
|参数|是否必选|参数说明|取值|
|--|--|--|--|
|ResetType|必选|复位类型。|字符串。<br>GracefulRestart：平滑复位。<br><div class="note"><span class="notetitle">[!NOTE] 说明</span><div class="notebody">在执行升级、升级生效、系统复位和恢复出厂操作时，再执行系统复位操作，系统复位操作会执行失败。</div></div>|
...
|NTPLocalServers|可选|用于NTP服务的本机IP地址。|字符串，IPv4地址。仅当ServerEnabled为true时参数有效<br><div class="note"><span class="notetitle">[!NOTE] 说明</span><div class="notebody">该字段在此版本上无效。</div></div>|
...
|ServerCert.CertificateSigningRequest|字符串|CSR信息。<br><div class="note"><span class="notetitle">[!NOTE] 说明</span><div class="notebody">导入服务器证书后，之前生成的CSR信息清除，此处显示为“null”。</div></div>|
|Actions|对象|可执行的操作。|
...
```http
PATCH https://10.10.10.10/redfish/v1/Systems/SecurityService/SecurityLoad
```
